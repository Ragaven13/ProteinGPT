...
GENERATED_CSV = "results/generated_vanilla_500.csv"
REAL_CSV = "data/processed/swissprot_clean.csv"
OUTPUT_CSV = "results/evaluation_vanilla_results.csv"
...
def main():
    print("Loading vanilla generated sequences...")
    generated_df = pd.read_csv(GENERATED_CSV)

    print("Loading real Swiss-Prot clean sequences...")
    real_df = pd.read_csv(REAL_CSV)

    generated_sequences = generated_df["generated_sequence"].dropna().tolist()
    real_sequences = real_df["sequence"].dropna().tolist()
    ...
    print("\nVanilla GPT Evaluation Results:")
    print(results_df)

    results_df.to_csv(OUTPUT_CSV, index=False)

    print(f"\nSaved vanilla evaluation results to: {OUTPUT_CSV}")
